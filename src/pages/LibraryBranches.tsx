...
const LibraryBranches = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await axios.get("http://localhost:5000/api/campuses-full");
    ...
    <div className="flex flex-col min-h-screen bg-slate-50">
      ...
      <section className="relative min-h-[400px] flex items-center justify-center overflow-hidden bg-[#0f2918]">
        <div className="relative z-20 text-center px-4 max-w-4xl">
          <div className="inline-flex items-center gap-2 px-3 py-1 bg-white/90 border border-yellow-600 rounded-full mb-6">
            <Wifi className="w-3.5 h-3.5 text-yellow-700" />
            <span className="text-yellow-700 text-xs font-bold uppercase">Campus Locations</span>
          </div>
          <h1 className="text-4xl md:text-6xl font-extrabold text-white mb-6">Library Network</h1>
        </div>
      </section>
                ...
                {campus.libraries?.map((lib: any) => (
                  <div key={lib.id} className="group bg-white rounded-2xl shadow-sm hover:shadow-xl transition-all border border-gray-100 overflow-hidden">
                    <div className="flex justify-between items-center p-5 bg-gradient-to-r from-green-50 to-white">
                      ...
                      {lib.isMain && <span className="text-xs bg-yellow-400 px-3 py-1 rounded-full font-bold">Main Branch</span>}
                    </div>
                    <div className="p-6">
                      ...
                      <div className="flex justify-between items-center mt-6">
                        <div>
                          <p className="text-xs text-gray-400 font-semibold uppercase">Seating</p>
                          <p className="text-xl font-bold text-[#1d4e2f]">{lib.capacity.toLocaleString()}</p>
                        </div>
                        <div className="w-10 h-10 flex items-center justify-center rounded-full bg-gray-100 group-hover:bg-[#1d4e2f] group-hover:text-white transition">
                          <ArrowRight size={18} />
                        </div>
                      </div>
                    </div>
...
const StatCard = ({ icon, value, label }: any) => (
  <div className="bg-white p-7 rounded-3xl shadow-xl text-center border border-gray-100">
    <div className="text-[#1d4e2f] flex justify-center mb-3">{React.cloneElement(icon, { size: 36 })}</div>
    <div className="text-4xl font-extrabold text-[#1d4e2f]">{value}</div>
    <div className="text-gray-500 font-semibold text-sm mt-1 uppercase">{label}</div>
  </div>
);
